...
# initialize dropped_patients file before can use print_to_drop
...
def create_linker_table(sedd, sid_ed, sid):
    ...
    print_to_drop(f"Initial Patient Number:{linker_table.shape[0]}")
...
def create_code_lookup_table(sedd, sasd, sid, linker_table):
    ...
    for dataset in dataset_list:
        if dataset["dataset"].shape[0] > 0: #Ignore empty datasets
