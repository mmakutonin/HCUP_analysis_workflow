...
import pandas as pd
import numpy as np
from utility_functions import print_to_drop

procedure_codes = { #currently no procedures, change for new procedure
    "cpt_codes": [],
    "ICD-10-procedures": []
}
diagnosis_codes = [ #currently for Suicide
    "T1491XA", #Injury of unspecified body region due to suicide attempt init encounter
    "R45851", #suicidal ideation
    "R4588", #non-suicidal self-harm
    "X7..XXA", "X8..XXA", #intentional self-harm, initial encounter
    "T3[6-9].2XA", "T3[6-9]..2A", "T4..2XA", "T4..*2A", "T50.2XA", "T50..2A", #drug poisoning, intentional self-harm
    "T71..2A" #Asphyxiation, intentional self-harm
]
...
init_visit_datasets = {
    ...
    "sasd": False,
    "sid": False,
    "sid_ed": False #rows in SID that represent patients admitted from the ED.
}
...
code_category_dict = {
    ...
    "Prior self-harm": ["Z915", "T1491XD", "T1491XS"],
    "Prior trauma or abuse": ["Z914", "Z6281", "T74[12356].X[DS]"],
    "Family history of substance abuse": ["Z81[34]"],
    "Family history of alcohol abuse": ["Z811"],
    "Substance Abuse": ["F1.1"],
    "Substance Dependence": ["F1.2"],
...
def enrich_disposition_categories(linker_table):
    non_transfer = linker_table[
        (linker_table["disposition_code"] != 26) &
        (linker_table["disposition_code"] != 42) &
        (linker_table["disposition_code"] != 50) 
    ]
    print_to_drop(f"Patients not transferred on index visit: {non_transfer.shape[0]}")
    return linker_table[
        (linker_table["disposition_code"] == 26) |
        (linker_table["disposition_code"] == 42) |
        (linker_table["disposition_code"] == 50) 
...
de_col_values = {
    de_col_keys[1]: ["Boarded Over 24h", "Boarded less than 24h"],
}
def data_enrichment_function(sedd, sasd, sid, sid_ed, codes, linker_table):
    ...
    linker_table[de_col_keys[0]] = linker_table.join(sedd, on="initial_record_id", rsuffix="_r")["length_of_stay"]
    linker_table[de_col_keys[1]] = (linker_table[de_col_keys[0]] >= 1).map({
...
demographic_tables = [
    {
        ...
        "save_filepath": f"../tables/{de_col_keys[0]} Table 1.csv",
    ...
    },
    {
        "key": de_col_keys[1],
        "query_string": "`Cost (USD)` >= 0", #Universal filter
        "save_filepath": f"../tables/{de_col_keys[1]} Table 1.csv",
        "has_outcome_crosscomparison": False,
        # "outcome_crosscomparison": [
        #     {
        #         "save_filepath": "../tables/table name.csv",
