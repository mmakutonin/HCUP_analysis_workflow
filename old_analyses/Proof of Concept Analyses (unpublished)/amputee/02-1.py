...
for dataset in ["sedd", "sid", "sasd"]:
    for year in ["2016", "2017", "2018"]:
        ...
        #filter out non-numerical (invalid) visit_links
        data_file = data_file.loc[~data_file['visit_link'].str.contains("\\D")].astype(died_reference["dtypes"])
        deaths = data_file.join(index, how="inner", on="visit_link").query(f"max_year >={year} and Died == 1")
...
print_to_drop(f"dropped {invalid_deaths.shape[0]} invalid patient(s) due to death on initial record.")
print_to_drop(f"dropped {np.logical_not(deaths.index.isin(summary_table.index)).sum()} invalid patient(s) due to death from unrelated cause.")
