...
from analysis_variables import de_col_keys, de_col_values, demographic_tables, code_category_dict, numeric_de_col_keys
...
for key in de_col_keys:
    if key not in numeric_de_col_keys:
...
category_dict = {
    ...
    **{value: 'Clinical Pathway' for value in numeric_de_col_keys}
}
...
def create_summary(groupby_col, save_filepath, tb, filter_criteria="`Cost (USD)` >= 0"): #cost should always be positive, making this a universal filter
    ...
    num_full_dataset = dem_dataset.join(comorbidities, how="left").query(filter_criteria)\
        .fillna(0).astype("int").join(full_dataset.join(comorbidities, how="left")[groupby_col], rsuffix="r_").groupby(groupby_col)
