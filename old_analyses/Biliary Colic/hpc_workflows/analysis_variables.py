...
de_col_keys = [
    ...
    "Outpatient Ambulatory Surgical Management"
]
de_col_values = {
    ...
    de_col_keys[6]: ["Ambulatory Cholecystectomy", "Non-Ambulatory Cholecystectomy"]
    }
def data_enrichment_function(sedd, sasd, sid, sid_ed, codes, linker_table):
    ...
    #Ambulatory Surgery
    linker_table[de_col_keys[6]] = code_linker(de_col_keys[2], codes=codes.query("amb_surg_flag == True")).map({
...
demographic_tables = [
    {
        "key": de_col_keys[0],
        "query_string": "`Cost (USD)` >= 0", #cost should always be positive, making this a universal filter
    ...
    },
    {
        ...
        "save_filepath": f"../tables/Table 1 Complicated {de_col_keys[0]}.csv",
        "has_outcome_crosscomparison": True,
        "outcome_crosscomparison": [
            {
                "save_filepath": "../tables/Cost of Admission vs Cost of Surgery.csv",
                "outcome_variable": "Cost (USD)",
                "groupby_row": de_col_values[de_col_keys[2]] #groupby_col is always the "key" attribute above
            }
        ]
    },
    {
...
logreg_targets = {
    'Complicated Colic - Given Admission - Immediate Cholecystectomy vs Others': \
    ...
    'Complicated Colic - Given Admission - Delayed Cholecystectomy vs None': \
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][0]) &
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][0]) & \
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][1]) & \
            (dataset[de_col_keys[0]] == de_col_values[de_col_keys[0]][0]),
            de_col_keys[2]
        ].eq(de_col_values[de_col_keys[2]][2]),
    'Complicated Colic - Given Admission - Delayed Cholecystectomy vs Delayed Emergency': \
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][0]) &
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][0]) & \
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][3]) & \
            (dataset[de_col_keys[0]] == de_col_values[de_col_keys[0]][0]),
            de_col_keys[2]
        ].eq(de_col_values[de_col_keys[2]][2]),
    'Complicated Colic - Given Discharge - No Surgery vs Others': \
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][0]) &
            (dataset[de_col_keys[0]] == de_col_values[de_col_keys[0]][1]),
            de_col_keys[2]
        ].eq(de_col_values[de_col_keys[2]][3]),
    'Complicated Colic - Given Discharge - Delayed Surgery vs Delayed Emergency': \
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][0]) &
            (dataset[de_col_keys[0]] == de_col_values[de_col_keys[0]][1]) &
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][3]) ,
    ...
    'Complicated Colic - Discharge vs Admission': \
        lambda dataset: dataset.loc[
            dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][0],
            de_col_keys[0]
        ].eq(de_col_values[de_col_keys[0]][1]),
    'Uncomplicated Colic - Discharge vs Admission':\
        lambda dataset: dataset.loc[
            dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][1],
            de_col_keys[0]
        ].eq(de_col_values[de_col_keys[0]][1]),
    'Uncomplicated Colic - Admission vs Discharge':\
        lambda dataset: dataset.loc[
            dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][1],
            de_col_keys[0]
        ].eq(de_col_values[de_col_keys[0]][0]),
    'Uncomplicated Colic - Given Discharge - No Surgery vs Others':\
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][1]) & \
            (dataset[de_col_keys[0]] == de_col_values[de_col_keys[0]][1]),
            de_col_keys[2]
        ].eq(de_col_values[de_col_keys[2]][3]),
    'Uncomplicated Colic - Given Admission - Immediate Surgery vs Others':\
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][1]) & \
            (dataset[de_col_keys[0]] == de_col_values[de_col_keys[0]][0]),
            de_col_keys[2]
        ].eq(de_col_values[de_col_keys[2]][0]),
    'Uncomplicated Colic - Given Admission - Delayed Surgery vs No Surgery':\
        lambda dataset: dataset.loc[
            (dataset[de_col_keys[1]] == de_col_values[de_col_keys[1]][1]) & \
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][0]) & \
            (dataset[de_col_keys[2]] != de_col_values[de_col_keys[2]][1]) & \
...
outcome_cols = [
    ...
    # "Obtained Cholecystectomy",
    {"name": "Post-Index Visit Cholecystitis", "type": "string", "positive_class": "New Cholecystitis"}
]
...
chart_plotting = [
    #complicated - surgery
    {
        ...
        'title': 'Cost of Delayed Cholecystectomy vs Non-Surgical Management for Initially Discharged Patients',
        'true_label': 'Non-Surgical Management',
        'false_label': 'Delayed Cholecystectomy',
        'logreg_key': 'Complicated Colic - Given Discharge - No Surgery vs Others',
    },
    {
        'metric':'ED Readmissions',
        'y_axis_label': 'Repeat ED Visits per Patient',
        'title': 'Repeat ED Visit Rates for Delayed Cholecystectomy vs Non-Surgical Management for Initially Discharged Patients',
        'true_label': 'Non-Surgical Management',
        'false_label': 'Delayed Cholecystectomy',
        'logreg_key': 'Complicated Colic - Given Discharge - No Surgery vs Others',
    },
    {
        ...
        'title': 'Repeat Hospitalization Rates for Delayed Cholecystectomy vs Non-Surgical Management for Initially Discharged Patients',
        'true_label': 'Non-Surgical Management',
        'false_label': 'Delayed Cholecystectomy',
        'logreg_key': 'Complicated Colic - Given Discharge - No Surgery vs Others',
    },
    #complicated - admission
    {
        'metric':'Cost',
        'y_axis_label': 'Cost (USD)',
        'title': 'Cost of Initial Admission vs Discharge for Complicated Biliary Colic',
        'true_label': 'Discharged on Index Visit',
        'false_label': 'Admitted on Index Visit',
        'logreg_key': 'Complicated Colic - Discharge vs Admission',
        'forecast_length': 30
    },
    {
        'metric':'ED Readmissions',
        'y_axis_label': 'Repeat ED Visits per Patient',
        'title': 'Repeat ED Visit Rates of Initial Admission vs Discharge for Complicated Biliary Colic',
        'true_label': 'Discharged on Index Visit',
        'false_label': 'Admitted on Index Visit',
        'logreg_key': 'Complicated Colic - Discharge vs Admission',
    },
    {
        'metric':'Inpatient Readmissions',
        'y_axis_label': 'Repeat Hospitalizations per Patient',
        'title': 'Repeat Hospitalization Rates of Initial Admission vs Discharge for Complicated Biliary Colic',
        'true_label': 'Discharged on Index Visit',
        'false_label': 'Admitted on Index Visit',
        'logreg_key': 'Complicated Colic - Discharge vs Admission',
    },
    #uncomplicated
    {
        ...
        'true_label': 'Discharged on Index Visit',
        'false_label': 'Admitted on Index Visit',
        'logreg_key': 'Uncomplicated Colic - Discharge vs Admission',
        'forecast_length': 30
    },
    {
        'metric':'ED Readmissions',
