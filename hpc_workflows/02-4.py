...
def enrich_care_costs_by_quarter(analysis_name:str):
    ...
    summary_enhanced = load_file("summary_enhanced.pickle", analysis_name)
    sid_costs = load_file("sid_costs.pickle", analysis_name)
    sedd = load_file("filtered_sedd_data.pickle", analysis_name)
    ...
    print_to_drop(f"{len(drop_rows)} records dropped due to zero SID and SEDD costs.", analysis_name)

    negative_cost = summary_enhanced.query('Cost < 0').index
    print_to_drop(f"{len(negative_cost)} dropped due to negative cost.", analysis_name)
    ...
    pickle_file("summary_costs_enhanced.pickle", analysis_name, summary_enhanced)
    pickle_file("outcomes_by_quarter.pickle", analysis_name, outcomes_by_quarter)